...
class XDPMCPServer {
            ...
            try {
                switch (name) {
                    ...
                    case 'trino_execute_query':
                        return await this.handleTrinoExecuteQuery(args);
                    case 'trino_list_catalogs':
                        return await this.handleTrinoListCatalogs(args);
                    case 'trino_list_tables':
                        return await this.handleTrinoListTables(args);
                    case 'trino_describe_table':
                        return await this.handleTrinoDescribeTable(args);
                    case 'execute_and_monitor':
                        return await this.handleExecuteAndMonitor(args);
                    case 'register_manual_execution':
                        return await this.handleRegisterManualExecution(args);
                    default:
     ...
     */
    async handleTrinoExecuteQuery(params) {
     ...
     */
    async handleTrinoListCatalogs(params) {
     ...
     */
    async handleTrinoListTables(params) {
     ...
     */
    async handleTrinoDescribeTable(params) {
        console.error('🔍 Describing Trino table...');
        try {
            const result = await this.xdpClient.getTrinoTableColumns(params.dataplane, params.catalog, params.schema, params.table);
     ...
     */
    async handleRegisterManualExecution(params) {
     ...
     */
    formatTrinoQueryResponse(result, query) {
        ...
        if (data.length > 0) {
            // Table header
            response += '| ' + columns.map((c) => c.name).join(' | ') + ' |\n';
     ...
     */
    formatTrinoCatalogsResponse(result) {
        ...
        result.result.data.forEach((row, index) => {
     ...
     */
    formatTrinoTablesResponse(result, catalog, schema) {
     ...
     */
    formatTrinoTableColumnsResponse(result, catalog, schema, table) {
